...
import {
  validateAndPreviewImportAction,
  executeImportAction,
  fetchGoogleSheetDataAction,
  ImportPreviewResult,
...
export default function ImportLeadsDialog({
  ...
  const handleTextSubmit = (rawText: string) => {
    try {
      setLoading(true);
      setError(null);
      const parsed = parseImportCsvText(rawText);
      setFile(null);
      setSourceName('Pasted Text Data');
      setHeaders(parsed.headers);
      setRawRows(parsed.rows);
      setMapping(autoDetectColumnMapping(parsed.headers));
      setStep('MAPPING');
  ...
  const handleProceedToPreview = async () => {
    if (!mapping.nameCol || !mapping.phoneCol) {
      setError('Please map both Name and Phone Number columns.');
      return;
    }
    try {
      setLoading(true);
      setError(null);
      const defaultSource = isGoogleSheet ? 'Google Sheets' : 'Manual Import';
  ...
  const handleDownloadSample = () => {
    const csv =
      'data:text/csv;charset=utf-8,Name,Phone,Email,Course/Interest,Source,Notes\nRahul Sharma,9876543210,rahul@example.com,Full Stack Web Dev,Facebook Ads,Morning batch\n';
    const a = Object.assign(document.createElement('a'), {
      href: encodeURI(csv),
      download: 'leads_sample_template.csv',
    });
            ...
            <div
              className={`p-2 rounded-xl shrink-0 ${
                isGoogleSheet
                  ...
                  : 'bg-[#2563EB]/10 text-[#2563EB] dark:bg-[#2563EB]/20 dark:text-blue-400 border border-blue-200/50 dark:border-blue-900/50'
              }`}
            >
              {isGoogleSheet ? <FileSpreadsheet className="w-5 h-5" /> : <UploadCloud className="w-5 h-5" />}
            </div>
            <div className="min-w-0">
              <h3 className="text-sm sm:text-base font-bold text-foreground">
                {isGoogleSheet ? 'Google Sheets Import' : 'Manual Lead Import'}
              </h3>
              <p className="text-xs text-muted-foreground font-medium truncate">
                {step === 'DROPZONE' &&
                  (isGoogleSheet
                    ? 'Paste your Google Sheets link to fetch and map leads'
                    : 'Upload CSV, Excel (.xlsx), or paste data manually')}
                {step === 'MAPPING' && 'Match your spreadsheet columns with lead fields'}
                {step === 'PREVIEW' && 'Review validation results before import'}
                {step === 'DONE' && (isGoogleSheet ? 'Google Sheet import complete' : 'Manual import complete')}
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-muted-foreground hover:text-foreground rounded-lg hover:bg-muted/60 transition-colors shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
